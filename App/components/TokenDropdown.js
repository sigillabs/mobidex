    ...
    return (
      <View>
        <TouchableOpacity onPress={this.props.onPress}>
          <Card wrapperStyle={{ flex: 1, flexDirection: "row", justifyContent: "flex-start", alignItems: "center", }}>
            <Text>{this.props.title} is </Text>
            <Text style={{ fontWeight: "bold" }}>{this.props.token.symbol}</Text>
          ...
          <List containerStyle={{marginBottom: 20}}>
            {
              this.props.tokens.map((token, index) => (
                <ListItem
                  ...
                  title={token.symbol}
                  titleStyle={{ color: this.props.token.address === token.address ? "blue" : "black" }}
                />
              ))
            }
          </List>
